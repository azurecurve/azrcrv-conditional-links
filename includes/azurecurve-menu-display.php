...
if ( ! function_exists( 'azrcrv_display_azurecurve_menu' ) ) {
	function azrcrv_display_azurecurve_menu() {
		...
		echo '<div id="azrcrv-general" class="wrap">';
			...
			$tab_active_label = esc_html__( 'Active Plugins', 'azrcrv-cl' );
			$active_plugins   = '';
		...
		foreach ( $plugin_array as $plugin_name => $plugin_details ) {
				...
				<table class="form-table azrcrv-settings">
					...
					<tr>
						...
						<td scope="row" colspan=2>
							...
							<p>' .
								sprintf( esc_html__( '%1$s was one of the first plugin developers to start developing for ClassicPress; all plugins are available from %2$s and are integrated with the %3$s plugin for fully integrated, no hassle, updates.', 'azrcrv-cl' ), '<strong>azurecurve | Development</strong>', '<a href="https://development.azurecurve.co.uk/classicpress-plugins/">azurecurve | Development</a>', '<a href="https://directory.classicpress.net/plugins/update-manager/">Update Manager</a>' )
							. '</p>
							<p>' .
								sprintf( esc_html__( 'The %s plugins active on your site are:', 'azrcrv-cl' ), '<strong>azurecurve | Development</strong>' )
							. '</p>
			...
			$tab_other_label = esc_html__( 'Other Available Plugins', 'azrcrv-cl' );
			$other_plugins   = '';
		...
		if ( $countofplugins == 0 ) {
			$other_plugins .= sprintf( esc_html__( 'Congratulations! You\'re using all of the %s plugins.', 'azrcrv-cl' ), 'azurecurve | Development' );
		}
				...
				<table class="form-table azrcrv-settings">
					...
					<tr>
						...
						<td scope="row" colspan=2>
							...
							<p>' .
								sprintf( esc_html__( 'The other plugins available from %s are:', 'azrcrv-cl' ), '<strong>azurecurve | Development</strong>' )
							. '</p>
